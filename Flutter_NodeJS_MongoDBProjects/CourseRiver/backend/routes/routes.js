    ...
    try {
        ...
        if (exist) {
            ...
            res.status(200).json({name:newUser.name,
                email:newUser.email,image:newUser.image,token,_id:newUser._id,admin:newUser.isAdmin,code:201});
        ...
        if (exist) {
            b.compare(password,exist.password).then((matched)=>{
                ...
                return res.status(200).json({name:exist.name,
                   image:exist.image,email:exist.email,token,code:201,admin:exist.isAdmin});
                }else{
...
router.post('/searchcourse', (req, res) => {
    ...
    Courses.find({ $and:[{courseName: { $regex: coursePattern } },{isAccepted:true}]})
        .then(course => {
...
router.get('/getUserCourse', reqLogin,async(req, res) => {
    await Courses.find({ratedBy:{$in:[req.user._id]}})
    .sort('-createdAt')
    .populate("comments","commentedText commentedBy")
        .then((course) => {
            res.status(201).json({data:course});
...
router.put('/removeRating/:id', reqLogin,async(req, res) => {
    console.log(req.body);
    const ress = await Courses.findByIdAndUpdate(req.params.id, {
        $pull: {
            ratedBy: req.user._id
        },
        courseRatings:req.body.newRatings
